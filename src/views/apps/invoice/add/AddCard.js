...
const AddCard = props => {
  ...
  const handlePreviewInvoice = formData => () => {
    ...
    dispatch(previewInvoice(formData))
      .then(response => {
        console.log('preview invoice', response)
        if (response?.payload?.success === true) {
          router.push('/apps/invoice/preview/view')
        } else {
          ...
          <Grid container spacing={6}>
            ...
            <Grid item xl={3} md={4} xs={12}>
              ...
              Send Invoice
                  ...
                  <Button fullWidth sx={{ mb: 3.5 }} variant='outlined' onClick={handlePreviewInvoice(values)}>
                    Draft and Preview
                  </Button>
