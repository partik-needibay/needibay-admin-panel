...
import { postQuotation, previewQuotation } from 'src/store/apps/quotations'
...
const AddCard = props => {
  ...
  const schema = yup.object().shape({
    ...
    invoiceId: yup.number().required('Quotation Id is required'),
  ...
  const handlePreviewQuotation = formData => () => {
    ...
    dispatch(previewQuotation(formData))
      .then(response => {
        console.log('preview invoice', response)
        if (response?.payload?.success === true) {
          router.push('/apps/quotation/preview/view')
        } else {
          toast.error('Quotation preview failed.')
        }
      })
      .catch(error => {
  ...
  const handleFormSubmit = formData => {
    ...
    dispatch(postQuotation(formData))
      .then(response => {
        if (response?.payload?.success === true) {
          toast.success('Quotation generated successfully!!')
          router.push('/apps/quotation/list')
        } else {
          toast.error('Quotation generation failed.')
        }
      })
      .catch(error => {
          ...
          <Grid container spacing={6}>
            ...
            <Grid item xl={3} md={4} xs={12}>
              ...
              Send Quotation
                  ...
                  <Button fullWidth sx={{ mb: 3.5 }} variant='outlined' onClick={handlePreviewQuotation(values)}>
                    Preview
                  </Button>
