...
import { selectVendorDocuments } from 'src/store/apps/vendor'
...
import vendorDocList from 'src/data/vendorDocList'
...
const DocumentUpload = () => {
  ...
  useEffect(() => {
    dispatch(selectVendorDocuments(documents))
  }, [documents])
  ...
  const handlePageBlockCode = async (file, docType) => {
    let fileObj = {}
    let fileArr = []
    fileObj.file = file
    fileObj.docType = docType
    fileArr.push(fileObj)
    setDocuments([...documents, fileObj])
  }
          ...
          <Grid container>
            ...
            <Grid item lg={4}>
              <Select
                ...
                defaultValue={'None'}
                name='pageBlockCode'
                onChange={e => handlePageBlockCode(file, e.target.value)}
              >
                <MenuItem value={''} selected>
                  None
                </MenuItem>
                {vendorDocList?.map(item => {
